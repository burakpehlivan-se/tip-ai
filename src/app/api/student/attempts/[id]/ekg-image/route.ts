...
/**
 * Sahibi doğrulanmış denemenin eşleştirilmiş PTB-XL EKG görüntüsünü döndürür.
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const [src] = await getDb()
      ...
      .limit(1);
    if (!src) return NextResponse.json({ error: "Bu vaka için EKG kaydı yok." }, { status: 404 });

    const filePath = resolveEkgImagePath(src.imageIndex);
